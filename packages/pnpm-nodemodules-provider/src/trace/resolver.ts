...
import { Installer, InstallTarget, PackageProvider } from '../install/installer'
...
export class Resolver {
  ...
  addCustomProvider(name: string, provider: any) {
  ...
  resolveBuiltin(
    ...
    | string
    | { target: InstallTarget; alias: string; subpath?: '.' | `./${string}` }
    | undefined {
    for (const provider of Object.values(this.providers)) {
      if (!(provider as any).resolveBuiltin) continue
      const builtin = (provider as any).resolveBuiltin.call(
        this,
        specifier,
        this.env,
      )
  ...
  async getExportResolution(
    ...
    if (originalSpecifier[0] === '#') {
      ...
      for (const curTarget of targets) {
        try {
          if (
            (await this.finalizeResolve(curTarget, false, null, pkgUrl)) ===
    ...
    if (pcfg.exports !== undefined && pcfg.exports !== null) {
      if (typeof pcfg.exports === 'string') {
        ...
        try {
          if (
            (await this.finalizeResolve(url, false, null, pkgUrl)) ===
            pkgUrl + subpath.slice(2)
          )
            return '.'
        } catch {}
...
async function legacyMainResolve(
  ...
  throw new JspmError(
    `Unable to resolve ${
      main ? main + ' in ' : ''
    }${pkgUrl} resolving ${originalSpecifier}${importedFrom(parentUrl)}.`,
    'MODULE_NOT_FOUND',
  )
}
