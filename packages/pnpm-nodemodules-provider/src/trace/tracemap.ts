...
import { JspmError, throwInternalError } from '../common/err'
import { parsePkg } from '../install/package'
// @ts-ignore
import {
  ImportMap,
  IImportMap,
...
import { isBuiltinScheme, isMappableScheme, Resolver } from './resolver'
    ...
    do {
      ...
      await Promise.all(
        modules.map(async (module) => {
          await this.visit(
            module,
            { mode: 'existing', static: this.opts.static, toplevel: true },
            this.mapUrl.href,
          )
        }),
      )
